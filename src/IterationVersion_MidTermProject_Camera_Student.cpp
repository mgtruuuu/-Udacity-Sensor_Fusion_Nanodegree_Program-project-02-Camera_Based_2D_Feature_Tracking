...
int main(int argc, const char* argv[]) {
    ...
    std::vector<Detector> detectors{ Detector::SHITOMASI, Detector::HARRIS, Detector::FAST, Detector::BRISK, Detector::ORB, Detector::AKAZE, Detector::SIFT };
    std::vector<Descriptor> descriptors{ Descriptor::FREAK, Descriptor::BRIEF, Descriptor::BRISK, Descriptor::ORB, Descriptor::AKAZE, Descriptor::SIFT };

        for (auto det{ detectors.begin() }; det != detectors.end(); ++det) {
            for (auto des{ descriptors.begin() }; des != descriptors.end(); ++des) {


                if (*det == Detector::AKAZE && *des == Descriptor::FREAK)       continue;

                if (*det == Detector::SIFT && *des == Descriptor::ORB)          continue;
                
                if (*des == Descriptor::AKAZE) {
                    if (*det != Detector::AKAZE)
                        continue;
                }
                ...
                //// Choose one between SHITOMASI, HARRIS, FAST, BRISK, ORB, AKAZE and SIFT.
                Detector detectorType{ *det };

                //// Choose one between FREAK, BRIEF, BRISK, ORB, AKAZE and SIFT.
                Descriptor descriptorType{ *des };

                constexpr Matcher matcherType{ Matcher::MAT_BF };       // MAT_BF, MAT_FLANN

                //// Choose one between DES_BINARY and DES_HOG
                DescriptorOption descriptorOptionType;
                if (descriptorType != Descriptor::SIFT)
                    descriptorOptionType = DescriptorOption::DES_BINARY;
                else
                    descriptorOptionType = DescriptorOption::DES_HOG;
